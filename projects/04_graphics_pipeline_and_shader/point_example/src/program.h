...
public:
    static ProgramUPtr Create(
        const std::vector<ShaderPtr> &shaders); // 링크에 성공하면 Opengl program object를 생성. 실패하면 메모리 할당해제(uniqueptr)
...
private:
    Program() {}
    bool Link(
        const std::vector<ShaderPtr> &shaders); // 두 개의 shader를 입력받아서 프로그램을 링크
